...
class GoogLeNet(nn.Module):
    def __init__(self, in_channels=3, num_classes=1000, conv_block=None):
        ...
        self.conv2 = self.conv_block(64, 64, kernel_size=1)
        self.conv3 = self.conv_block(64, 192, kernel_size=3, padding=1)
...
if __name__ == '__main__':
    ...
    model1 = googlenet(num_classes=2).eval()
    model2 = GoogLeNet(num_classes=2).eval()

    model1_l, model2_l = [], []
    for name, p in model1.named_parameters():
        model1_l.append(name)
    print('---------------------------------')
    for name, p in model2.named_parameters():
        model2_l.append(name)
      
    print(set(model1_l) - set(model2_l))